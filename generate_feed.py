...
"""
generate_feed.py
Hämtar live-feed från Webnode, filtrerar norsk-kategoriprodukter,
...
headers = {"Cache-Control":"no-cache", "Pragma":"no-cache","Expires":"0","User-Agent":"Mozilla/5.0 (compatible; feed-updater/1.0)"}
...
for attempt in range(1, FETCH_ATTEMPTS+1):
    ...
    try:
        ...
        if ("<item" in last_resp_text) and (("g:id" in last_resp_text) or ("<g:id" in last_resp_text) or ("<id>" in last_resp_text)):
            print("[fetch] OK - feed looks valid.")
            break
        else:
            print("[fetch] Warning: feed fetched but no items found. Retrying...")
